...
public class KE_TA_Structure extends BaseIndustry implements MarketImmigrationModifier {

    public static float BASE_BONUS = 10f;
    public static float ALPHA_CORE_BONUS = 0.05f;
    public static float IMPROVE_BONUS = 0.05f;
    ...
    public static String SPECIALMARKET = "KE_TAMarket";
    public static String CORPORATE_FACTION = "KE_TACorporation";
...
//        market.getStability().modifyFlat(getModId(), -STABILITY_PENALTY, getNameForModifier());
        market.getAccessibilityMod().modifyFlat(getModId(0), 0.15f, getNameForModifier());
...
//        market.getStability().unmodifyFlat(getModId());
    ...
    protected void addStabilityPostDemandSection(TooltipMakerAPI tooltip, boolean hasDemand, IndustryTooltipMode mode) {
        ...
        tooltip.addPara("Accessibility:%s", opad, h, "+15%");
    }

    @Override
    protected void addRightAfterDescriptionSection(TooltipMakerAPI tooltip, IndustryTooltipMode mode) {
        if (market.isPlayerOwned() || currTooltipMode == IndustryTooltipMode.ADD_INDUSTRY) {
            tooltip.addPara("Establishes a chartered Flicker Aviation liaison office and opens the Flight Bazaar, a storefront specializing in carrier hulls, LPCs, fighter support gear, and aerospace hardware.", 10f);
        }
    }
    ...
    protected void applyAlphaCoreModifiers() {
        market.getAccessibilityMod().modifyFlat(getModId(1), ALPHA_CORE_BONUS, "Alpha Core (" + getNameForModifier() + ")");
    }
    @Override
    ...
    protected void addAlphaCoreDescription(TooltipMakerAPI tooltip, AICoreDescriptionMode mode) {
        ...
        if (mode == AICoreDescriptionMode.INDUSTRY_TOOLTIP) {
            ...
            text.addPara(pre + "reduces upkeep cost by %s, reduces demand by %s unit, and increases accessibility by %s.", 0f, highlight,
        ...
        tooltip.addPara(pre + "reduces upkeep cost by %s, reduces demand by %s unit, and increases accessibility by %s.", opad, highlight,
                "" + (int) ALPHA_UPKEEP_REDUCTION + "%", "" + DEMAND_REDUCTION,
                str);
    ...
    protected void applyImproveModifiers() {
        if (isImproved()) {
            market.getAccessibilityMod().modifyFlat(getModId(2), IMPROVE_BONUS,
